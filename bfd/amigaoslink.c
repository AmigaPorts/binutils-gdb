     ...
     asymbol **symbols)
{
  ...
  if (data == NULL)
      data = bfd_malloc (input_section->rawsize);
...
is_dwarf_reloc (arelent *r, asection *sec)
{
    ...
    return (strcmp(n, ".debug_info")    == 0 ||
            strcmp(n, ".debug_abbrev")  == 0 ||
            strcmp(n, ".debug_line")    == 0 ||
            strcmp(n, ".debug_aranges") == 0 ||
            strcmp(n, ".debug_frame")   == 0 ||
            strcmp(n, ".debug_str")     == 0);
}
...
//  printf("%s %s: %x\n", sym->name, target_section->name, r->howto->type);
  ...
  switch (r->howto->type)
    ...
    case H_ABS32:
AbsReloc:
	/* DWARF: section-relative relocations */
	  if (is_dwarf_reloc(r, target_section))
		{
			relocation = sym->value + r->addend + target_section->output_offset;
		}
	  else if (hard_reloc)
...
//		  printf("stab: %s %s %d+%d=%d\n", sym->section->name, sym->name, sym->value, target_section->output_offset, relocation);
      ...
      else /* Same section */
	{
	  ...
	  /* my_add_to() below adds this displacement to the field, which
	     gas filled with the target's offset within its section plus
	     any addend, for strong and weak targets alike.  */
	  relocation = sym->value + target_section->output_offset
...
extern bool _bfd_generic_link_write_global_symbol(struct generic_link_hash_entry *, void *);
extern bool _bfd_generic_link_output_symbols(bfd *, bfd *, struct bfd_link_info *, size_t *);
     ...
     struct bfd_link_info *info)
{
  ...
  struct bfd_link_hash_entry *h2 =
    bfd_link_hash_lookup (info->hash, "___a4_init2", false, false, true);
      ...
      if (bs && !ds)
        {
          ...
          section->name = ".data";
          section->flags |= (SEC_ALLOC | SEC_LOAD | SEC_DATA
                             | SEC_HAS_CONTENTS | SEC_IN_MEMORY);

          section->contents = bfd_alloc (abfd, 8);
          if (!section->contents)
            return false;
          memset (section->contents, 0, 8);
  ...
  for (o = abfd->sections; o != (asection *) NULL; o = o->next)
    {
      ...
      if (o->reloc_count > 0)
        {
          bfd_size_type amt = o->reloc_count * sizeof (arelent *);
  ...
  /* SBF: check if some offset is given, then all relocs are resolved using
     output_section->vma. */
  for (o = abfd->sections; o != NULL; o = o->next)
    if (o->vma != 0
        && (o->prev == NULL
            || o->prev->size + o->prev->vma != o->vma)
        && 0 == strncmp (".text", o->name, 5))
      {
           ...
           p = p->next)
        {
          switch (p->type)
            ...
            case bfd_symbol_reloc_link_order:
              if (!amiga_reloc_link_order (abfd, info, o, p))
                return false;
              break;
