...
static bool amiga_bfd_copy_private_section_data PARAMS ((bfd *,
	sec_ptr, bfd *, sec_ptr, struct bfd_link_info *));
...
reloc_howto_type howto_table[R__MAX] =
{
  ...
  HOWTO(H_ABS16, 0,  2, 16, false, 0, complain_overflow_bitfield, 0, "RELOC16",    false, 0x0000ffff, 0x0000ffff, false),
  HOWTO(H_ABS8,  0,  1,  8, false, 0, complain_overflow_bitfield, 0, "RELOC8",     false, 0x000000ff, 0x000000ff, false),
  HOWTO(H_PC32,  0,  4, 32, true,  0, complain_overflow_signed,   0, "RELRELOC32", false, 0xffffffff, 0xffffffff, true ),
  HOWTO(H_PC16,  0,  2, 16, true,  0, complain_overflow_signed,   0, "RELRELOC16", false, 0x0000ffff, 0x0000ffff, true ),
  HOWTO(H_PC8,   0,  1,  8, true,  0, complain_overflow_signed,   0, "RELRELOC8",  false, 0x000000ff, 0x000000ff, true ),
  HOWTO(H_SD32,  0,  4, 32, false, 0, complain_overflow_bitfield, 0, "DREL32",     false, 0xffffffff, 0xffffffff, false),
  HOWTO(H_SD16,  0,  2, 16, false, 0, complain_overflow_bitfield, 0, "DREL16",     false, 0x0000ffff, 0x0000ffff, false),
  HOWTO(H_SD8,   0,  1,  8, false, 0, complain_overflow_bitfield, 0, "DREL8",      false, 0x000000ff, 0x000000ff, false),
  HOWTO(H_PC26,  0,  4, 26, true,  0, complain_overflow_signed,   0, "RELRELOC26", false, 0x03fffffc, 0x03fffffc, true )
};
	  ...
	  if (len <= 16)
	    {
	      ...
	      if (bfd_read (name, len<<2, abfd) != len<<2)
		return false;
	      name[len<<2] = 0;
	      last_name = name;
#ifdef DEBUG_AMIGA
	  ...
	  if (last_name)
	      ...
	      if (stab_pos && stabstr_pos)
		{
		  ...
		  if (bfd_seek (abfd, stabstr_pos, SEEK_SET))
		    return false;
		  if (bfd_read (stabstrdata, stabstr_size, abfd) != stabstr_size)
		    return false;
		  ...
		  if (bfd_seek (abfd, stab_pos, SEEK_SET))
		    return false;
		  if (bfd_read (stabdata, stab_size, abfd) != stab_size)
		    return false;
  ...
  bfd *abfd)
{
  ...
  if (get_long (abfd, &hunk_type) && hunk_type == HUNK_DEBUG)
	...
	if (bfd_seek(abfd, -4, SEEK_CUR) >= 0)
	  amiga_handle_cdb_hunk (abfd, hunk_type, -1, 0, sz * 4);
    }
...
/* If hunk_size==-1, then we are digesting a HUNK_UNIT */
{
  ...
  if (HUNK_NAME == HUNK_VALUE (hunk_type)) /* get name */
      ...
      if (len != 0)
	{
	  ...
	  if (bfd_read (current_name, len, abfd) != len)
	    return false;
	  ...
	  if (0 == strncmp(".debug_", current_section->name, 7))
	    {
		  ...
		  get_long (abfd, &real_len);
		  if (((real_len + 7) & ~3) == len)
		    {
			  len = real_len;
			  current_section->alignment_power = 0;
		    }
		  else if (bfd_seek (abfd, cpos, SEEK_SET))
		    return false;
	    }
...
//		  len -= sizeof(buf);
//		  if (len > amiga_per_section(current_section)->disk_size || bfd_read (buf, sizeof(buf), abfd) != sizeof(buf))
//		    return false;
//		  abfd->symcount++;
//
//		  /* fallthrough */
      ...
      /* skip the contents */
      if ((secflags & SEC_HAS_CONTENTS) && bfd_seek (abfd, (len + 3) & ~3, SEEK_CUR))
	return false;
	  ...
	  if (GL (&buf[0]) == ZMAGIC) /* GNU DEBUG HUNK */
	  ...
	  else   /* Not ZMAGIC -> treat as merged DWARF2 container */
	    {
		 ...
		 the linker script.  */
	      ...
	      bfd_size_type dwarf_size = len;   /* already known */
	      bfd_vma dwarf_filepos = bfd_tell (abfd) - sizeof(buf);     /* start of hunk data */
		 ...
		 later in amigaread.c once the symbol table is available.  */
	      ...
	      if (bfd_seek (abfd, len - sizeof(buf), SEEK_CUR))
	    	  return false;
	...
	case HUNK_EXT:
	  ...
	  for (;;)
		...
		break;
	      ...
	      if (bfd_read (nlt + 1, len<<2, abfd) != len << 2)
		return false;
  ...
  while (nb)
    {
      ...
      if (bfd_write ((void *)out, 4*i, abfd) != 4*i)
	return false;
      nb -= i;
    }
  return true;
  ...
  while (nb)
    {
      ...
      if (bfd_write ((void *)out, 2*i, abfd) != 2*i)
	return false;
      nb -= i;
    }
  return true;
...
/* Amiga hunk fields are fixed-width even when bfd_size_type is wider than
   unsigned long, as it is on 64-bit Windows hosts.  Convert the value rather
   than passing a pointer to an incompatible representation.  */
static bool
...
static bool
amiga_pack_dwarf_sections (bfd *abfd, asection *dwarf, asection *tail)
{
  ...
  if (AMIGA_DATA(abfd)->IsLoadFile)
    {
      ...
      for (q = abfd->sections, p = q ? q->next : 0; p; p = q->next)
	{
	  ...
	  if (dwarf)
	    {
	      if (!amiga_pack_dwarf_sections (abfd, dwarf, q))
	        return false;
	    }
...
#define CAN_WRITE_OUTSYM(sym) (sym!=NULL && sym->section && \
	  ...
	  for (ii = 0; ii < bfd_get_symcount(abfd); ii++) /* Translate every symbol */
	      ...
	      if (CAN_WRITE_OUTSYM (sym))
		{
		  ...
		  if (bfd_write ((void *) &data, sizeof(data), abfd) != sizeof(data))
		    return false;
		}
	    }
	  ...
	  for (ii = 0; ii < bfd_get_symcount(abfd); ii++)
	      ...
	      if (CAN_WRITE_OUTSYM (sym))
		{
		  ...
		  /* Write string tab */
		  if (bfd_write (sym->name, len, abfd) != len)
		    return false;
		}
	    }
	  ...
	  n[0] = 0;
	  ii = (4 - (str_size & 3)) & 3;
	  if (ii && bfd_write ((void *) n, ii, abfd) != ii)
	    return false;
     ...
     unsigned long value)
{
  ...
  if (!write_longs (n, 1, abfd))
    return false;
  if (bfd_write (name, l, abfd) != l)
    return false;
  n[0] = 0;
  l = (4 - (l & 3)) & 3;
  return (l && bfd_write ((void *)n, l, abfd) != l ? false : true);
}
  ...
  for (object = arch->archive_head; object; object = object->archive_next)
      ...
      while (remaining)
	{
	  ...
	  if (amt > remaining)
	    amt = remaining;
	  errno = 0;
	  if (bfd_read (buf, amt, object) != amt)
	    {
	      if (bfd_get_error () != bfd_error_system_call)
		bfd_set_error (bfd_error_malformed_archive);
	      return false;
	    }
	  if (bfd_write (buf, amt, arch) != amt)
	    return false;
	  remaining -= amt;
	}
    }
	  ...
	  if (n[0] == HUNK_DEBUG && 0 == strncmp(".debug", section->name, 6))
	    {
		  if (!write_bfd_size_long (section->size, abfd))
			  return false;
		  if (bfd_write ((void *)section->contents, amiga_per_section(section)->disk_size - 4, abfd) !=
		      		  amiga_per_section(section)->disk_size - 4)
			  return false;
	    }
	  else if (bfd_write ((void *)section->contents, amiga_per_section(section)->disk_size, abfd) !=
  ...
  if (datadata_relocs)
      ...
      for (s = 0; s < data_sec->reloc_count; s++)
	{
	  ...
	  if (insection->output_section == data_sec)
		...
		if (!write_bfd_size_long (r->address, abfd))
		  return false;
	    }
	}
    }
	  ...
	  reloc_count -= relocs;
		...
		if (!write_bfd_size_long (r->address, abfd))
		  return false;
			    ...
			    {
		if (!write_bfd_size_word (r->address, abfd))
		  return false;
	      }
	      if (--relocs == 0)
		break;
  ...
  if (offset+count > disk_size)
    {
      ...
      if (bfd_read (location, disk_size-offset, abfd) != disk_size-offset)
      return false;
...
amiga_slurp_symbol_table (bfd *abfd)
{
  ...
  for (section=abfd->sections; section!=NULL; section=section->next)
      ...
      for (asect->amiga_symbols=asp; get_long (abfd, &l) && l; asp++)
	{
	  ...
	  if ((asp->symbol.name = bfd_alloc (abfd, len+1))==NULL)
	    return false;
	  if (bfd_read ((void *)asp->symbol.name, len, abfd) != len)
	    return false;
  ...
  if (stab && stabstr)
	...
	return false;
      if (bfd_read (stabstrdata, astabstr->disk_size, abfd) != astabstr->disk_size)
	return false;
	...
	return false;
      if (bfd_read (stabdata, astab->disk_size, abfd) != astab->disk_size)
	return false;
...
amiga_load_stab_symbols (bfd *abfd)
{
  ...
  if (!stab || !stabstr)
	...
	return 0;
      ...
      if (bfd_read (stab, amiga_data->symtab_size, abfd) != amiga_data->symtab_size)
	return 0;

      stabstr = (unsigned char *)bfd_alloc(abfd, amiga_data->stringtab_size);
      bfd_seek(abfd, adata(abfd).str_filepos, SEEK_SET);
      if (bfd_read (stabstr, amiga_data->stringtab_size, abfd) != amiga_data->stringtab_size)
	return 0;
    }
  ...
  bfd *abfd)
{
  ...
  for (csym = defsyms; syms; syms = syms->next)
	...
	return false;
      ...
      if (!symblock)
	return false;
      if (bfd_read (symblock, syms->size, abfd) != syms->size)
	return false;
...
bfd *abfd)
{
  ...
  ared = calloc (1, sizeof (struct areltdata));
  if (ared == NULL)
    return NULL;
  ...
  switch (len)
    {
    default:
      if (bfd_read (ared->filename, len, abfd) != len)
	return NULL;
      ared->filename[len] = '\0';
      /* strip path part */
...
#define amiga_find_nearest_line_with_alt _bfd_nosymbols_find_nearest_line_with_alt
...
#define amiga_finalize_section_relocs _bfd_generic_finalize_section_relocs
